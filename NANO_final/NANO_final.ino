...
void loop() {
  ...
  while (digitalRead(startBut) == LOW) {
    ...
    if (scanSuccess) {  // 成功掃到條碼
      // while ((millis() - startTime) < delayTime) {  // 掃到條碼後等待時間
      //   int endButState = digitalRead(endBut);
      //   if (endButState == LOW) {  // 等待中持續判斷 endBut 是否被按下
      //     endButtonPressed = true;
      //     break;
      //   }
      // }
      delay(500);
      binArrayHasElement = 1;
    }
  } 
  if (binArrayHasElement) {  // binArray 中有資料
    if (endButtonPressed == true || digitalRead(endBut) == LOW) {
